...
<body>
    ...
    <table class="table">
        ...
        <tbody>
            @php $groupNumber = 1; $grandTotalQty = $grandTotal = 0; @endphp
            @foreach ($data as $index => $po)
                        ...
                        @php
                            ...
                            $grandTotalQty += $po->TPCHORDDETA_ITMQT;
                            $grandTotal += (($po->TPCHORDDETA_ITMQT * $po->TPCHORDDETA_ITMPRC_PER) + $po->totalTax);
                        @endphp
            ...
            <thead>
                <tr>
                    @foreach ($approvalList as $approval)
                        <td style="text-align: center;">{{ $approval['remarks'] }}</td>
                    @endforeach
                </tr>
            </thead>
            <tbody>
